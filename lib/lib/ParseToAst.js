...
var fastIsWhitespace_1 = require("./fastIsWhitespace");
var stringLiteralSPELStyle_1 = __importDefault(require("./stringLiteralSPELStyle"));
 ...
 */
var parse = function (input, graceful, allowWeirdQuoteCharacters) {
    ...
    var utils = {
        ...
        whitSpc: function () {
            while (index < input.length &&
                (0, fastIsWhitespace_1.fastCharCodeIsWhitespace)(input.charCodeAt(index))) {
                index += 1;
            }
            return true;
        },
        ...
        stringLiteral: function () {
            var match = (0, stringLiteralSPELStyle_1.default)(input.slice(index), allowWeirdQuoteCharacters);
            if (match === -1) {
                if (graceful) {
                    return -1;
                }
                throw new CustomErrors_1.ParsingError(input, index, "Non-terminating quoted string");
            }
            if (match) {
                index = index + match[1];
                return match[0];
    ...
    var expression = function () {
        ...
        if (utils.char("?")) {
            if (utils.char(":")) {
                ...
                if (!exp2 && !graceful) {
                    throw new CustomErrors_1.ParsingError(input, index, "Expected expression after elvis (?:)");
                }
                return {
                    type: "Elvis",
                    expression: exp1,
                ...
                if (((exp2 = expression()) && utils.char(":") && (exp3 = expression())) ||
                    ...
                    throw new CustomErrors_1.ParsingError(input, index, "Incomplete Ternary");
                }
            }
        }
        else {
        ...
        utils.zeroOrMore(function () {
            ...
            if (utils.char("|")) {
                if (utils.char("|")) {
                    if ((right = logicalAndExpression()) || graceful) {
                        ...
                        throw new CustomErrors_1.ParsingError(input, index, "No right operand for ||");
                    }
                }
                else {
                    throw new CustomErrors_1.ParsingError(input, index, "Missing Character |");
                }
            }
            // Now opOr
            var backtrack = index;
            ...
            if ((keyword === null || keyword === void 0 ? void 0 : keyword.toLowerCase()) === "or") {
                if ((right = relationalExpression()) || graceful) {
                    ...
                    throw new CustomErrors_1.ParsingError(input, index, "No right operand for OR");
                }
            }
            else {
                index = backtrack;
        ...
        utils.zeroOrMore(function () {
            ...
            if ((keyword === null || keyword === void 0 ? void 0 : keyword.toLowerCase()) === "and") {
                if ((right = relationalExpression()) || graceful) {
                    ...
                    throw new CustomErrors_1.ParsingError(input, index, "No right operand for AND");
                }
            }
            else {
                index = backtrack;
    ...
    */
    var relationalExpression = function () {
        ...
        if (utils.char(">")) {
            ...
            if (utils.char("=")) {
                ...
                throw new CustomErrors_1.ParsingError(input, index, "Assignment not allowed");
            }
        }
        else {
            ...
            if (keyword === "matches") {
                if ((right = sumExpression()) || graceful) {
                    ...
                    throw new CustomErrors_1.ParsingError(input, index, "No right operand for 'matches'");
                }
            }
                ...
                if ((right = sumExpression()) || graceful) {
                    ...
                    throw new CustomErrors_1.ParsingError(input, index, "No right operand for 'between'");
                }
            }
            else {
                if (keyword && !["and", "or"].includes(keyword === null || keyword === void 0 ? void 0 : keyword.toLowerCase())) {
                    if (!graceful) {
                        throw new CustomErrors_1.ParsingError(input, index, "Not an Operator", "\"".concat(keyword, "\" is not an operator"));
                    }
                }
                index = backtrack_1;
            }
    ...
    var productExpression = function () {
        ...
        utils.zeroOrMore(function () {
            ...
            if (utils.char("*")) {
                if (!left) {
                    throw new CustomErrors_1.ParsingError(input, index - 1, "No left operand for *");
                }
        ...
        if (utils.char(".")) {
            return (methodOrProperty(false) ||
                (function () {
                    ...
                    throw new CustomErrors_1.ParsingError(input, index, "Expected property after .");
                })());
        }
        var backtrack = index;
    ...
    var nullSafeIndex = function () {
        ...
        if (utils.char("?") && utils.char("[")) {
            if ((innerExpression = expression()) || graceful) {
                ...
                throw new CustomErrors_1.ParsingError(input, index, "Expected ]");
            }
            else {
                if (utils.char("]")) {
                    index -= 1;
                    throw new CustomErrors_1.ParsingError(input, index, "Expression expected in []");
                }
                throw new CustomErrors_1.ParsingError(input, index, "Expected ]");
            }
        }
    ...
    var methodOrProperty = function (nullSafeNavigation) {
        ...
        if ((ident = utils.identifier())) {
            ...
            if (utils.char("(")) {
                ...
                throw new CustomErrors_1.ParsingError(input, index, "Expected ) for method call");
            }
            else {
                index = fnbacktrack;
                return {
    ...
    var functionOrVar = function () {
        ...
        if (utils.char("#") && (ident = utils.identifier())) {
            ...
            if (utils.char("(")) {
                ...
                throw new CustomErrors_1.ParsingError(input, index, "Expected ) for function call");
            }
            else {
                index = fnbacktrack;
                return {
        ...
        var result = utils.firstOf(function () {
            ...
            if (utils.char("?") && utils.char("[")) {
                if (utils.whitSpc() &&
                    ...
                    throw new CustomErrors_1.ParsingError(input, index, "Expected ] for selection expression ?[");
                }
            }
            return null;
        }, function () {
    ...
    var projection = function () {
        ...
        if (utils.char("!") && utils.char("[")) {
            if ((exp = expression()) && utils.char("]")) {
                ...
                throw new CustomErrors_1.ParsingError(input, index, "Expected ] for Projection expression ![");
            }
        }
        else {
            index = backtrack;
    ...
    var parenExpression = function () {
        ...
        if (utils.char("(")) {
            ...
            if (utils.char(")")) {
                index -= 1;
                throw new CustomErrors_1.ParsingError(input, index, "Expected expression in ()");
            }
            throw new CustomErrors_1.ParsingError(input, index, "Expected )");
        }
        return null;
    ...
    if (index !== input.length && !graceful) {
        throw new CustomErrors_1.ParsingError(input, index, "Expression Remaining", "input remaining: ".concat(input.slice(index)));
    }
    if (result === null && !graceful) {
        throw new CustomErrors_1.ParsingError(input, index, "Generic");
    }
    return result;
};
exports.parse = parse;
